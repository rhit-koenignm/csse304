...
; This method filters the first set for values that are not contained in the subtractme set
(define (set-subtract set subtractme)
  (filter (lambda (x) (not (contains? x subtractme))) set))
...
(define (sublist? big little)
  (cond [(null? little) #t]
        [(null? big) #f]
        [else (or (sublist-helper big little) (sublist-helper (cdr big) little))]))

(define (sublist-helper big little)
  (cond [(null? little) #t]
        [(null? big) #f]
        [(equal? (car big) (car little)) (sublist-helper (cdr big) (cdr little))]
        [else #f]))
...
(define (remove-depth depth slist)
  (cond [(null? slist) '()]
        [(symbol? (car slist)) (cons (car slist) (remove-depth depth (cdr slist)))]
        [(list? (car slist)) (if (<= depth 1) (append (list (car slist)) (remove-depth depth (cdr slist)))
                                              (append (remove-depth (- depth 1) (car slist)) (remove-depth depth (cdr slist))))]
        [(<= depth 1) (list (car slist) (remove-depth depth (cdr slist)))]
        [else (append (list (remove-depth (- depth 1) (car slist))) (remove-depth depth (cdr slist)))]))
        
; Reference from slist code done in class
    ...
    (cond [(null? slist) '()]
	  ...
	  [else ; car is an s-list
	   (append (flatten (car slist))
	       (flatten (cdr slist)))])))
        ; (and (list? (car slist)) (> depth 1)) (append (remove-depth (- depth 1) (car slist)) (remove-depth depth (cdr slist)))]
        ; [else (append (car slist) (remove-depth depth (cdr slist)))]))
...
(define make-bag
  (lambda ()
    (let ([bag '()])
      (lambda (msg  . args)        
        (case msg 
          [(add) (set! bag (cons (car args) bag))]
          [(count) (length (filter (lambda (x) (equal? x (car args))) bag))]
          [else (errorf 'bag "illegal message to bag object: ~a" msg)])))))
